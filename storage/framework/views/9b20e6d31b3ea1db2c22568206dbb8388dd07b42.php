...
function saveOrderImages(rowId,order,model,currVal){    
	if(rowId != '' && order != '' && model != '' && currVal != '' && $.isNumeric(rowId) && $.isNumeric(order) && $.isNumeric(currVal)){
		$.ajax({
			type:'POST',
			url:"<?php echo e(url('/admin/update-new-order')); ?>", 
			async:false,
			headers:{
...
function saveOrder(rowId,order,model,currVal){
	if(rowId != '' && order != '' && model != '' && currVal != '' && $.isNumeric(rowId) && $.isNumeric(order) && $.isNumeric(currVal)){
		$.ajax({
			type:'POST',
			url:"<?php echo e(url('/admin/update-order')); ?>", 
			async:false,
			headers:{
				'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			},
...
<?php /**PATH G:\xampp-8.2\htdocs\laraval_ecommerce_admin\resources\views/element/admin/jquery.blade.php ENDPATH**/ ?>