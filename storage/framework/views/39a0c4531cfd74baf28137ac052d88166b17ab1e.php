        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    ...
                    <p class="text-subtitle text-muted">Categories list.</p>
                </div>
            ...
            <div class="card">
               <!--begin::Card body-->
                <div class="card-body">
                    ...
                    <a href="<?php echo e(url('/admin/add-category')); ?>" class="btn icon btn-sm btn-outline-success float-end">Add New Category</a>
                    <?php endif; ?>
                </div>
                <!--end::Card body-->
            </div>
                                ...
                                <table class="table mb-0">
                                    <thead class="thead-dark">
                                        <tr>
                                            ...
                                            <th>IMAGE</th>
                                            <th>DESCRIPTION</th>
                                            <?php if(Session::get('admin_type') == 'Admin'): ?><th>STATUS</th><?php endif; ?>
                                            <th>CREATED</th>
                                            <?php if(Session::get('admin_type') == 'Admin'): ?><th>ACTION</th><?php endif; ?>
                                        </tr>
                                    </thead>
    ...
    <div class="modal fade" id="uploadCsvModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <a href="<?php echo e(url('/storage/sample-csv/category.csv')); ?>" target="_blank" class="btn btn-success">Download Sample CSV</a>
...
function filterData(type = null){
	...
	$.ajax({
		...
		url: "<?php echo e(url('/admin/categories_paginate')); ?>",
...
function exportData(){
	...
	$.ajax({
		headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
		type: "POST",
		url: "<?php echo e(route('exports.category')); ?>",
		data: $('#searchForm').serialize(),
...
function UploadCSV(){
	if($.trim($('#upload_csv_file').val()) != ''){
		...
		$.ajax({
			headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			url: "<?php echo e(route('imports.category')); ?>",
...
<?php echo $__env->make('layout.admin.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH G:\xampp-8\htdocs\aayush-bharat-admin\resources\views//admin/categories/index.blade.php ENDPATH**/ ?>