...
function updatePrice(value,id,field){
	if(value != '' && id != '' && field != ''){
		$.ajax({
			type: 'POST',
			url: "<?php echo e(url('/admin/change-price')); ?>",
...
function changeProductStatus(status,table,rowID){
	if(status == 3){
		...
		$.ajax({
			type: 'POST',
			url: "<?php echo e(url('/admin/change-product-status')); ?>",
			headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
...
function acceptProducts(){
    ...
    if (willDelete) {
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type: 'POST',
            url: "<?php echo e(url('/admin/accept-products')); ?>",
            data: {productIDs:productIDs},
            success: function(msg){
                if(msg == "Success"){
                    swal({
    ...
    if(declineReason == ""){
        ...
        if (willDelete) {			
            $.ajax({
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: 'POST',
                url: "<?php echo e(url('/admin/decline-products')); ?>",
...
<?php /**PATH /home/vqtxcve1uvhl/mailwiz.365wah.com/resources/views/element/admin/jquery.blade.php ENDPATH**/ ?>