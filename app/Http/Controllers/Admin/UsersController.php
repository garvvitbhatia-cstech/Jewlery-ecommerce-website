...
class UsersController extends Controller{
    ...
    public function addPage(Request $request){
		...
		if($request->input()){
			...
			if($validator->fails()){
                ...
                if(!self::$User->ExistingRecord($request->input('email')) && !self::$User->ExistingRecordPhone($request->input('mobile'))){
		...
		return view('/admin/users/add-page',compact('country_list'));
    }
    ...
    public function editPage(Request $request, $row_id){
        ...
        if($request->input()){
			...
			if($validator->fails()){
                ...
                if(self::$User->ExistingRecordUpdate($request->input('email'), $RowID) || self::$User->ExistingRecordPhoneUpdate($request->input('mobile'), $RowID)){
        ...
        if(isset($rowData->id)){
            ...
            return view('/admin/users/edit-page',compact('rowData','row_id','country_list'));
    ...
    public function changeBidderStatus(Request $request){
		...
		if($tableName != "" && $rowID != "" && $status != "" && is_numeric($rowID) && is_numeric($status)){
			...
			DB::table($tableName)->where(array('id' => $rowID))->update(array('is_bidder' => $newStatus));
    ...
    public function countryList(){
        return self::$Country->where('status',1)->orderBy('ordering')->get();
    }
